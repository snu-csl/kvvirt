...
int main(int argc, char *argv[])
{
    ...
    memcpy(buf, &key_len, sizeof(u_int8_t));
    memcpy(buf + sizeof(u_int8_t), key, key_len);

        printf("buffer contents length %d pattern len %d:\n%s\n", value_len, value_pattern_len, buf);
        printf("last data info %d\n", buf[value_len - 2]);
    }

    ret = nvme_kv_store(space_id, fd, nsid, key, key_len, buf, (int)value_len, (int)offset, option);
    if (ret) {
