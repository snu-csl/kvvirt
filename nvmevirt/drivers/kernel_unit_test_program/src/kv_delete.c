...
void usage(void)
{
    printf("[Usage] kv_delete -d device_path -k key_string -z space_id -s len -l offset -i (idempotent)\n");
}
...
int main(int argc, char *argv[])
{
    ...
    int offset = 0;
    int value_len = 0;
    while((opt = getopt(argc, argv, "d:k:z:s:l:i")) != -1) {
        switch(opt) {
            case 'd':
                dev = optarg;
            break;
    ...
    ret = nvme_kv_delete(space_id, fd, nsid, key, key_len, value_len, offset, option);
    if (ret) {
