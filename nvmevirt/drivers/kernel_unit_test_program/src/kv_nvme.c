...
int nvme_kv_batch(int space_id, int fd, unsigned int nsid,
...
int nvme_kv_delete(int space_id, int fd, unsigned int nsid,
		const char *key, int key_len, int offset, int len, enum nvme_kv_delete_option option)
{
	int ret = 0;
	struct nvme_passthru_kv_cmd cmd;
	memset(&cmd, 0, sizeof (struct nvme_passthru_kv_cmd));
