...
uint64_t init_cache(struct cache* c, uint64_t tt_pgs, uint64_t dram_bytes) 
{
     ...
     */
    ht = (struct ht_section**) vmalloc_node(c->nr_valid_tpages * 
                                            sizeof(struct ht_section*),
                                            numa_node_id());
    total += c->nr_valid_tpages * sizeof(struct ht_section*);

    ht_mem = (struct ht_section *) vmalloc_node(c->nr_valid_tpages * 
                                                sizeof(struct ht_section), 
                                                numa_node_id());
    total += sizeof(struct ht_section) * c->nr_valid_tpages;
