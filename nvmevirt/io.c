...
static unsigned int __do_perform_io_kv(int sqid, int sq_entry)
{
    ...
    bool batch = cmd->common.opcode == nvme_cmd_kv_batch;
    bool off_read = false;
	...
	while (remaining) {
        ...
        if(write) {
            ...
            //char key[8];
            //if(io_size % 16 == 0) {
            //    NVMEV_DEBUG("Composition of this store key %llu sz %lu:\n", 
            //            *(uint64_t*) cmd->kv_store.key, io_size);
            //    for(int i = 0; i < io_size / 16; i++) {
            //        memcpy(key, (char*) (ptr + (i * 16)), 8);
            //        NVMEV_DEBUG("S COMP %llu %s %llu |", *(uint64_t*) cmd->kv_store.key,
            //                key, *(uint64_t*) (ptr + (i * 16) + 8));
            //    }
            //    NVMEV_DEBUG("\n");
            //}
            ...
            //char key[8];
            //if(io_size % 16 == 0) {
            //    NVMEV_DEBUG("Composition of this append key %llu sz %lu:\n", 
            //                 *(uint64_t*) cmd->kv_store.key, io_size);
            //    for(int i = 0; i < io_size / 16; i++) {
            //        memcpy(key, (char*) (ptr + (i * 16)), 8);
            //        NVMEV_DEBUG("A COMP offs %d iosz %lu %llu %s %llu |", i, io_size,
            ...
            //char key[8];
            //if(io_size % 16 == 0) {
            //    NVMEV_DEBUG("Composition of this read key %llu sz %lu:\n", 
            //            *(uint64_t*) cmd->kv_store.key, io_size);
            //    for(int i = 0; i < io_size / 16; i++) {
            //        memcpy(key, (char*) (ptr + (i * 16)), 8);
            //        NVMEV_DEBUG("OUT COMP %d %llu %s %llu |", i, *(uint64_t*) cmd->kv_store.key,
            //                key, *(uint64_t*) (ptr + (i * 16) + 8));
            //    }
            //    NVMEV_DEBUG("\n");
            //}
