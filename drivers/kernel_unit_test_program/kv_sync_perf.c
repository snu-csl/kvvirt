...
static void *do_io_worker(void *ctx)
{
    ...
    srand((t2.tv_sec - t1.tv_sec) * 1000000000);
    ...
    char tmp_value[value_len];
    for(int j = 0; j < value_len; j++) {
        tmp_value[j] = 'A' + (rand() % 26);
    }
    ...
    for (i = 0; i < num_io; i++) {
        ...
        sprintf(key_buf, "%0*d", key_len, keys[(start_num + i) % keycnt]); // rand() % keycnt);
        memcpy(tmp_key, key_buf, key_len);

        memcpy(tmp_value, &key_len, sizeof(u_int8_t));
        memcpy(tmp_value + sizeof(u_int8_t), tmp_key, key_len);
        //sprintf(tmp_value + sizeof(u_int8_t) + key_len, "VALUE");
        ...
        //        printf("Writing data %s klen %u\n", tmp_value, *(u_int8_t*) tmp_value);
        //}
        ...
        //if(cmd.opcode == nvme_cmd_kv_retrieve) {
        //        printf("Got data %s\n", tmp_value);
        //}
    }
...
int main(int argc, char *argv[])
{
    ...
    while((opt = getopt(argc, argv, "d:l:s:k:n:t:ORD")) != -1) {
        switch(opt) {
            case 'd':
                dev = optarg;
            break;
    ...
    srand(100);
    keys = (int*) malloc(sizeof(int) * keycnt);
