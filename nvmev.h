...
#define NVMEV_INFO(string, args...) BUG_ON(false); // printk(KERN_INFO "%s: [t: %d cycles: %llu] " string, NVMEV_DRV_NAME, current->pid, get_cycles(), ##args)
#define NVMEV_ERROR(string, args...) printk(KERN_INFO "%s: " string, NVMEV_DRV_NAME, ##args)
...
#define TIMER_START() ktime_t tstart, tend; tstart = ktime_get()
#define TIMER_STOP_MICROSECONDS(msg) \
        do { \
                    tend = ktime_get(); \
                    uint64_t total = ktime_to_ns(ktime_sub(tend, tstart)) / 1000; \
                    if(total > 50) { \
                    NVMEV_INFO("%s: %llu microseconds\n", (msg), ktime_to_ns(ktime_sub(tend, tstart)) / 1000); \
                    } \
                } while (0)
...
struct nvmev_dev {
    ...
    struct proc_dir_entry *proc_space;
