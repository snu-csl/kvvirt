...
skiplist *skiplist_init(){
	skiplist *point=(skiplist*)kzalloc(sizeof(skiplist), GFP_KERNEL);
	point->level=1;
#ifdef USINGSLAB
	//point->header=(snode*)slab_alloc(&snode_slab);
	point->header=(snode*)kmem_cache_alloc(snode_slab,KM_NOSLEEP);
#else
	point->header=(snode*)kzalloc(sizeof(snode), GFP_KERNEL);
#endif
	point->header->list=(snode**)kzalloc(sizeof(snode*)*(MAX_L+1), GFP_KERNEL);
...
snode *skiplist_insert_existIgnore(skiplist *list,KEYT key,ppa_t ppa,bool deletef){
...
#endif
	{
		...
		if(ppa==x->ppa){
			printk("%.*s ppa:%u",KEYFORMAT(key),ppa);
			printk("Should have aborted!!!! %s:%d\n", __FILE__, __LINE__);
		}
...
value_set **skiplist_make_valueset(skiplist *input, level *from,KEYT *start, KEYT *end){
	...
	for_each_sk(target,input){
		...
		if(b.bucket[target->value->length]==NULL){
			b.bucket[target->value->length]=(snode**)kzalloc(sizeof(snode*)*(input->size+1), GFP_KERNEL);
		}
	...
	for(int i=0; i<=NPCINPAGE; i++){
		if(b.bucket[i]) kfree(b.bucket[i]);
	}
	res[res_idx]=NULL;
	return res;
}
...
//   inf_free_valueset(x->value, FS_MALLOC_W);
	kfree(x->list);
#ifdef USINGSLAB
	//slab_kfree(&snode_slab,x);
	kmem_cache_kfree(snode_slab,x);
#else
	kfree(x);
#endif
	list->size--;
...
sk_iter* skiplist_get_iterator(skiplist *list){
	sk_iter *res=(sk_iter*)kzalloc(sizeof(sk_iter), GFP_KERNEL);
	res->list=list;
	res->now=list->header;
	return res;
}
...
void skiplist_clear(skiplist *list){
	...
	while(now!=list->header){
		...
		kfree(now->list);
#ifdef USINGSLAB
	//	slab_kfree(&snode_slab,now);
		kmem_cache_kfree(snode_slab,now);
#else
		kfree(now);
#endif
		now=next;
...
void skiplist_kfree(skiplist *list){
	if(list==NULL) return;
	skiplist_clear(list);
	kfree(list->header->list);
#ifdef USINGSLAB
	//slab_kfree(&snode_slab,list->header);
	kmem_cache_kfree(snode_slab,list->header);
#else
	kfree(list->header);
#endif
	kfree(list);
...
void skiplist_container_kfree(skiplist *list){
	if(list==NULL) return;
	snode *now=list->header->list[1];
	snode *next=now->list[1];
	while(now!=list->header){
...
void skiplist_print(skiplist *skip){
	...
	for_each_sk(temp,skip){
		printk("[lev:%d]%p\t",temp->level,temp);
		for(uint32_t i=0; i<temp->level; i++){
			printk("[%.*s] ", temp->key.len,temp->key.key);
		}
		printk("\n");
	}

	printk("max level ptr:");
	for(uint32_t i=skip->level; i>=1; i--){
		printk("%p ",skip->header->list[i]);
	}
	printk("\n");
...
skiplist *skiplist_divide(skiplist *in, snode *target){
	...
	if((origin_level!=in->level && origin_level!=res->level) ||(in->level!=0 && (in->header->list[in->level]==in->header || res->header->list[res->level]==res->header))){
		printk("origin_level:%d in->level:%d\n",origin_level,in->level);
		printk("res->header->list[res->level] %p, res->header %p\n",res->header->list[res->level],res->header);
		printk("in->header->list[in->level] %p, in->header %p\n",in->header->list[in->level],in->header);
		printk("skiplist_divide error!\n");
		printk("Should have aborted!!!! %s:%d\n", __FILE__, __LINE__);;
	}
