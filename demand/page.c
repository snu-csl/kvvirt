...
static int _do_bulk_mapping_update(blockmanager *bm, struct gc_table_struct **bulk_table, int nr_valid_pages) {
	sort(bulk_table, nr_valid_pages, sizeof(struct gc_table_struct *), lpa_compare, NULL);
...
int dpage_gc(blockmanager *bm) {
	...
	for (int i = 0; i < nr_valid_pages; i++) kfree(bulk_table[i]);
	kfree(bulk_table);
...
ppa_t get_dpage(blockmanager *bm) {
	...
	if (!d_active || bm->check_full(bm, d_active, MASTER_BLOCK)) {
		if (bm->pt_isgc_needed(bm, DATA_S)) {
			...
			printk("DATA GC - (valid/total: %d/%d)\n", nr_valid_pages, _PPS);
#endif
		} else {
...
int tpage_gc(blockmanager *bm) {
	...
	struct gc_table_struct **bulk_table = 
    (struct gc_table_struct **)kzalloc(sizeof(struct gc_table_struct *) * _PPS, GFP_KERNEL);
...
#ifdef PRINT_GC_STATUS
			printk("TRANS GC - (valid/total: %d/%d)\n", nr_valid_pages, _PPS);
#endif
		} else {
...
int validate_page(blockmanager *bm, ppa_t ppa, page_t type) {
	...
	if (unlikely(!rc)) printk("Should have aborted!!!! %s:%d\n", __FILE__, __LINE__);
	return rc;
}
