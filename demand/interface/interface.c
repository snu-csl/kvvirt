...
//	for(int i=0; i<1; i++){
//		if(self()==mp.processors[i].t_id){
//			_this=&mp.processors[i];
//		}
//	}
//
//	char thread_name[128]={0};
//	sprintk(thread_name,"%s","inf_main_thread");
//	setname_np(self(),thread_name);
//	while(1){
//		cl_grap(inf_cond);
//		if(mp.stopflag)
//			break;
...
//		mutex_init(&t->qm_lock,NULL);
//		t->qmanager=rb_create();
//#else
//		q_init(&t->req_q,QSIZE);
//#endif
//		create(&t->t_id,NULL,&p_main,NULL);
//	}
//
//
//	mutex_init(&mp.flag,NULL);
//	if(apps_flag){
//		bench_init();
...
//	for(int i=0; i<1; i++){
//		processor *t=&mp.processors[i];
//		//		mutex_unlock(&inf_lock);
//		while(tryjoin_np(t->t_id,(void**)&temp)){
//			cl_release(inf_cond);
//		}
//		//detach(t->t_id);
//		q_free(t->req_q);
...
//			break;
//		case FS_ITER_NXT_VALUE_T:
//			req->multi_value=(value_set**)kzalloc(sizeof(value_set*)*lengths, GFP_KERNEL);
//			for(i=0; i < lengths; i++){
//				req->multi_value[i]=inf_get_valueset(NULL,FS_GET_T,PAGESIZE);
//			}
//			req->num=lengths;
...
//			break;
//		default:
//			printk("error in inf_make_multi_req\n");
//			return false;
//	}
//	req->ppa=iter_id;
...
//	req->multi_key=(KEYT*)kzalloc(sizeof(KEYT)*num, GFP_KERNEL);
//	req->multi_value=(value_set**)kzalloc(sizeof(value_set*)*num, GFP_KERNEL);
//	for(int i=0; i<num; i++){
...
value_set *inf_get_valueset(PTR in_v, int type, uint32_t length){
	value_set *res=(value_set*)kzalloc(sizeof(value_set), GFP_KERNEL);
	//check dma alloc type
#ifdef DVALUE
	length=(length/PIECE+(length%PIECE?1:0))*PIECE;
#endif
	if(length==PAGESIZE) {
        res->value = kzalloc(PAGESIZE, GFP_KERNEL);
        res->dmatag = 0;
		//res->dmatag=kzalloc((void**)&(res->value),PAGESIZE, GFP_KERNEL);
		...
		v_cnt[length/PIECE]++;
		res->dmatag=-1;
		res->value=(PTR)kzalloc(length, GFP_KERNEL);
	}
	res->length=length;
