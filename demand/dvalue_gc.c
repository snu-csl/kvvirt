...
void __update_pt(struct demand_shard *shard, struct cmt_struct *cmt, 
                 lpa_t lpa, ppa_t ppa, int gc_line) {
    ...
    for(int i = 0; i < cmt->cached_cnt; i++) {
        if(cmt->pt[i].lpa == lpa) {
            ppa_t before = atomic_read(&cmt->pt[i].ppa);

            if(__grain2lineid(shard, before) != gc_line) {
                return;
            }

            atomic_cmpxchg(&cmt->pt[i].ppa, before, ppa);
            //if(__grain2lineid(shard, cmt->pt[i].ppa) != gc_line) {
            //    /*
            //     * Updated in __store while we were GCing.
            //     */
            //    return;
            //}
...
//            __update_pt(shard, cmt, lpa, ppas[i].new_ppa, gc_line);
...
//            prev = UINT_MAX;
//#else
//            prev = __already_read(read_ppas, read_ppa_cnt, 
//                                  atomic_read(&cmt->t_ppa));
//#endif
...
//            mark_grain_invalid(shard, PPA_TO_PGA(cmt->t_ppa, cmt->g_off), 
//                               cmt->len_on_disk);
//            atomic_store(&cmt->t_ppa, UINT_MAX);
//            cmt->t_ppa = UINT_MAX;
//
...
//
//        cmt->pt = cmt->pt_mem;
//        __update_pt(shard, cmt, ppas[i].lpa, ppas[i].new_ppa, gc_line);
//
...
//
//        while(i + 1 < nr_valid_grains && IDX(ppas[i + 1].lpa) == idx) {
//            __update_pt(shard, cmt, ppas[i + 1].lpa, ppas[i + 1].new_ppa, gc_line);
