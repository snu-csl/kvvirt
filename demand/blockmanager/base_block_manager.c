...
uint32_t base_destroy (struct blockmanager* bm){
	...
	drb_delete(p->seg_map,true);
	for(int i=0; i<BPS; i++){
		channel *c=&p->base_channel[i];
		q_free(c->free_block);
		mh_kfree(c->max_heap);
	}
	kfree(p->base_channel);
	kfree(p);
	return 1;
}

__block* base_get_block (struct blockmanager* bm, __segment* s){
	if(s->now+1>s->max) printk("Should have aborted here base_get_block.\n");
...
__gsegment* base_get_gc_target (struct blockmanager* bm){
	bbm_pri *p=(bbm_pri*)bm->private_data;
	__gsegment* res=(__gsegment*)kzalloc(sizeof(__gsegment), GFP_KERNEL);
	for(int i=0; i<BPS; i++){
...
int base_populate_bit (struct blockmanager* bm, ppa_t ppa){
	int res=1;
	bbm_pri *p=(bbm_pri*)bm->private_data;
...
int base_unpopulate_bit (struct blockmanager* bm, ppa_t ppa){
	int res=1;
	bbm_pri *p=(bbm_pri*)bm->private_data;
	uint32_t bn=GETBLOCKIDX(checker,ppa);
	uint32_t pn=GETPAGEIDX(ppa);
...
int base_erase_bit (struct blockmanager* bm, ppa_t ppa){
	int res=1;
	bbm_pri *p=(bbm_pri*)bm->private_data;
	uint32_t bn=GETBLOCKIDX(checker,ppa);
	uint32_t pn=GETPAGEIDX(ppa);
	...
	if(0<=ppa && ppa< _PPS*MAPPART_SEGS){
		if(b->invalid_number>_PPB){
            printk("Should have aborted here base_erase_bit.\n");
		}
	}
	return res;
}

bool base_is_valid_page (struct blockmanager* bm, ppa_t ppa){
	bbm_pri *p=(bbm_pri*)bm->private_data;
	uint32_t bn=GETBLOCKIDX(checker,ppa);
	uint32_t pn=GETPAGEIDX(ppa);
	uint32_t bt=pn/8;
...
bool base_is_invalid_page (struct blockmanager* bm, ppa_t ppa){
	return !base_is_valid_page(bm,ppa);
}

void base_set_oob(struct blockmanager* bm, char *data,int len, ppa_t ppa){
	bbm_pri *p=(bbm_pri*)bm->private_data;
	__block *b=&p->base_block[GETBLOCKIDX(checker,ppa)];
	memcpy(b->oob_list[GETPAGEIDX(ppa)].d,data,len);
}

char *base_get_oob(struct blockmanager*bm,  ppa_t ppa){
	bbm_pri *p=(bbm_pri*)bm->private_data;
