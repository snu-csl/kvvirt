...
static void fg_env_init(cache_t c_type, struct cache_env *const _env) {
	...
	_env->max_cached_tentries = (d_env.size / K) / (ENTRY_SIZE + 4 + sizeof(lpa_t));
...
static void fg_member_init(struct cache_member *const _member) {
	...
	for (int i = 0; i < cenv->nr_valid_tpages; i++) {
		...
		cmt[i]->is_cached = (bool *)kzalloc(EPP * sizeof(bool), GFP_KERNEL);
		cmt[i]->cached_cnt = 0;
		cmt[i]->dirty_cnt = 0;
	}
	_member->cmt = cmt;
...
static void fg_print_member(void) {
	printk("=====================");
	printk(" Cache Finish Status ");
	printk("=====================");

	printk("Max Cached tentries:     %d\n", cenv->max_cached_tentries);
	printk("Current Cached tentries: %d\n", cmbr->nr_cached_tentries);
	printk("\n");
}

static void fg_member_kfree(struct cache_member *_member) {
	for (int i = 0; i < cenv->nr_valid_tpages; i++) {
		q_free(_member->cmt[i]->retry_q);
		q_free(_member->cmt[i]->wait_q);
		kfree(_member->cmt[i]->is_cached);
		kfree(_member->cmt[i]);
	}
	kfree(_member->cmt);
...
/*	if (!cmt->is_cached[OFFSET(lpa)]) printk("Should have aborted!!!! %s:%d
, __FILE__, __LINE__);;
